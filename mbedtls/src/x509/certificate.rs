...
impl LinkedCertificate {
	...
	pub fn issuer_raw(&self) -> ::Result<Vec<u8>> {
		::private::alloc_vec_repeat(|buf, size| unsafe { x509_dn_gets(buf as _, size, &self.inner.issuer) }, false)
	}
	...
	pub fn subject_raw(&self) -> ::Result<Vec<u8>> {
		::private::alloc_vec_repeat(|buf, size| unsafe { x509_dn_gets(buf as _, size, &self.inner.subject) }, false)
	}
	...
	pub fn serial_raw(&self) -> ::Result<Vec<u8>> {
		::private::alloc_vec_repeat(|buf, size| unsafe { x509_serial_gets(buf as _, size, &self.inner.serial) }, false)
	}
	...
	pub fn verify(&mut self, trust_ca: &mut Certificate, err_info: Option<&mut String>) -> ::Result<()> {
		let mut flags = 0;
		let result = unsafe {
			x509_crt_verify(&mut self.inner, &mut trust_ca.inner, ptr::null_mut(), ptr::null(),
							&mut flags, None, ptr::null_mut())
		}.into_result();
...
impl<'r> UnsafeFrom<*const x509_crt> for Iter<'r> {
	unsafe fn from(ptr: *const x509_crt) -> Option<Iter<'r>> {
		if ptr.is_null() {
			...
			Some(Iter { next: ptr, r: PhantomData })
		}
	}
}
...
impl<'r> UnsafeFrom<*mut x509_crt> for IterMut<'r> {
	unsafe fn from(ptr: *mut x509_crt) -> Option<IterMut<'r>> {
		if ptr.is_null() {
			...
			Some(IterMut { next: ptr, r: PhantomData })
		}
	}
}
...
impl<'a> Builder<'a> {
	...
	pub fn write_der_vec<F: ::rng::Random>(&mut self, rng: &mut F) -> ::Result<Vec<u8>> {
		::private::alloc_vec_repeat(|buf, size| unsafe {
	...
	impl Test {
		...
		fn builder<'a>(&'a mut self) -> Builder<'a> {
			...
			b.subject_key(&mut self.key1)
				...
				.unwrap()
				.validity(Time::new(2000, 1, 1, 0, 0, 0).unwrap(),
...
";
	...
	const TEST_DER: &'static [u8] =
		&[];

	#[test]
	fn write_der() {
		let mut t = Test::new();
		let output = t.builder()
