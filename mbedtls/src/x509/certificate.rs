...
fn x509_time_to_time(tm: &x509_time) -> Result<Time> {
    ...
    if tm.year < 0 || tm.mon < 0 || tm.day < 0 || tm.hour < 0 || tm.min < 0 || tm.sec < 0 {
        return Err(Error::from(MbedErrorCode::X509InvalidDate));
    }

    Time::new(tm.year as u16, tm.mon as u8, tm.day as u8, tm.hour as u8, tm.min as u8, tm.sec as u8).ok_or(MbedErrorCode::X509InvalidDate.into())
}
...
impl Certificate {
    ...
    pub fn from_pem(pem: &[u8]) -> Result<MbedtlsBox<Certificate>> {
        ...
        if !(*cert).inner.next.is_null() {
            ...
            return Err(Error::from(MbedErrorCode::X509BadInputData));
        }
    ...
    pub fn version(&self) -> Result<CertificateVersion> {
        match self.inner.version {
            ...
            _ => Err(MbedErrorCode::X509InvalidVersion.into())
        }
    }
    ...
    pub fn extensions(&self) -> Result<Vec<Extension>> {
        ...
        yasna::parse_der(&self.extensions_raw()?, |r| {
            ...
            return Ok(());
        }).map_err(|_| Error::from(MbedErrorCode::X509InvalidExtensions))?;
...
impl<'a> Builder<'a> {
    ...
    pub fn subject(&mut self, subject: &str) -> Result<&mut Self> {
        match ::std::ffi::CString::new(subject) {
            Err(_) => Err(Error::from(MbedErrorCode::X509InvalidName)),
    ...
    pub fn subject_with_nul(&mut self, subject: &str) -> Result<&mut Self> {
        if subject.as_bytes().iter().any(|&c| c == 0) {
            ...
            Err(Error::from(MbedErrorCode::X509InvalidName))
        }
    }
...
impl MbedtlsBox<Certificate> {
    fn init() -> Result<Self> {
        unsafe {
            ...
            let inner = NonNull::new(inner).ok_or(Error::from(MbedErrorCode::X509AllocFailed))?;
...
-----END CERTIFICATE-----\0";
    ...
    fn verify_chain() {
        ...
        match res {
            ...
            Err(err) => assert_eq!(err, MbedErrorCode::X509CertVerifyFailed.into()),
        }
    ...
    fn revoked_cert_crl_test() {
        ...
        assert_eq!(
            ...
            MbedErrorCode::X509CertVerifyFailed.into()
        );
