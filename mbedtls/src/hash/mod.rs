...
impl Md {
	...
	pub fn update(&mut self, data: &[u8]) -> ::Result<()> {
		unsafe {
			try!(::mbedtls_sys::md_update(&mut self.inner,data.as_ptr(),data.len()).into_result());
		}
		Ok(())
	}
	...
	pub fn hmac(md: Type, key: &[u8], data: &[u8], out: &mut [u8]) -> ::Result<usize> {
		...
		unsafe {
			...
			try!(::mbedtls_sys::md_hmac(md.inner,key.as_ptr(),key.len(),data.as_ptr(),data.len(),out.as_mut_ptr()).into_result());
			Ok(olen)
		}
	}
}
