...
impl Ecdh {
	...
	pub fn calc_secret<F: ::rng::Random>(&mut self, shared: &mut [u8], rng: &mut F) -> ::Result<usize> {
		let mut olen = 0;
		unsafe { ecdh_calc_secret(&mut self.inner, &mut olen, shared.as_mut_ptr(), shared.len(), Some(F::call), rng.data_ptr()).into_result()? };
		Ok(olen)
	}
}
...
-----END PUBLIC KEY-----\0";
		...
		const DH_P192: &'static [u8] = &[0x80, 0x3d, 0x8a, 0xb2, 0xe5, 0xb6,
			0xe6, 0xfc, 0xa7, 0x15, 0x73, 0x7c, 0x3a, 0x82, 0xf7, 0xce, 0x3c,
			0x78, 0x31, 0x24, 0xf6, 0xd5, 0x1c, 0xd0];

		let mut k_pr = Pk::from_private_key(PRIVATE_P192, None).unwrap();
		let k_pb = Pk::from_public_key(PUBLIC_P192).unwrap();
		let mut out = [0; 192/8];
		let len = k_pr.agree(&k_pb, &mut out, &mut ::test_support::rand::test_rng()).unwrap();
