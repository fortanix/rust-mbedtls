...
use mbedtls_sys::{hmac_drbg_seed, hmac_drbg_seed_buf, hmac_drbg_set_prediction_resistance, hmac_drbg_reseed, hmac_drbg_update,
                  hmac_drbg_random, HMAC_DRBG_PR_OFF, HMAC_DRBG_PR_ON};
...
impl<'entropy> HmacDrbg<'entropy> {
	pub fn new<F: EntropyCallback>(md_info: ::hash::MdInfo,
	...
	pub fn set_prediction_resistance(&mut self, pr: bool) {
		unsafe {
			hmac_drbg_set_prediction_resistance(&mut self.inner,
			                                    if pr {
				                                    HMAC_DRBG_PR_ON
				                                   } else {
				                                    HMAC_DRBG_PR_OFF
