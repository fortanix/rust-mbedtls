...
impl HmacDrbg {
    ...
    pub fn prediction_resistance(&self) -> bool {
        if unsafe { hmac_drbg_get_prediction_resistance(&self.inner) } == HMAC_DRBG_PR_OFF {
            false
        } else {
            true
        }
    ...
    getter!(entropy_len() -> size_t = fn hmac_drbg_get_entropy_len);
    setter!(set_entropy_len(len: size_t) = hmac_drbg_set_entropy_len);
    getter!(reseed_interval() -> c_int = fn hmac_drbg_get_reseed_interval);
