...
impl CtrDrbg {
    ...
    pub fn prediction_resistance(&self) -> bool {
        if unsafe { ctr_drbg_get_prediction_resistance(&*self.inner) } == CTR_DRBG_PR_OFF {
            false
        } else {
            true
        }
    ...
    getter!(entropy_len() -> size_t = fn ctr_drbg_get_entropy_len);
    setter!(set_entropy_len(len: size_t) = ctr_drbg_set_entropy_len);
    getter!(reseed_interval() -> c_int = fn ctr_drbg_get_reseed_interval);
