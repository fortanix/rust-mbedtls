...
use mbedtls_sys::{ctr_drbg_seed, ctr_drbg_set_prediction_resistance, ctr_drbg_reseed, ctr_drbg_update, ctr_drbg_random,
                  CTR_DRBG_PR_OFF, CTR_DRBG_PR_ON};
...
mod private {
	...
	use mbedtls_sys::{ctr_drbg_context, ctr_drbg_init, ctr_drbg_free};
...
impl<'entropy> CtrDrbg<'entropy> {
	fn init() -> Self {
		CtrDrbg { boxed: Box::new(CtrDrbgInner::init()) }
	}
}
