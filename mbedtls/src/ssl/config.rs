...
impl NullTerminatedStrList {
    ...
    pub fn new(list: &[&str]) -> Result<Self> {
        ...
        for item in list {
            ret.c.push(::std::ffi::CString::new(*item).map_err(|_| Error::from(MbedErrorCode::SslBadInputData))?.into_raw());
        }
...
impl Config {
    ...
    pub fn set_min_version(&mut self, version: Version) -> Result<()> {
        let minor = match version {
            ...
            Version::Tls1_1 => 2,
            Version::Tls1_2 => 3,
            _ => { return Err(Error::from(MbedErrorCode::SslBadHsProtocolVersion)); }
        };
