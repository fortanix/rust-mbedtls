...
use mbedtls_sys::types::raw_types::{c_char, c_uchar, c_int, c_uint, c_void};
use mbedtls_sys::types::size_t;
use mbedtls_sys::*;

use error::IntoResult;
use x509::{LinkedCertificate, Crl, Profile, VerifyError, certificate};
...
impl<'c> Config<'c> {
	...
	pub fn set_dh_params(&mut self, params: &[u8]) -> ::Result<()> {
		...
		unsafe { ssl_conf_dh_param_ctx(&mut self.inner, (&mut ctx).into()).into_result().map(|_| ()) }
	}

	pub fn set_ca_list<C: Into<&'c mut LinkedCertificate>>(&mut self, list: Option<C>, crl: Option<&'c mut Crl>) {
		unsafe {
			ssl_conf_ca_chain(&mut self.inner,
			                  list.map(Into::into)
				                  .map(Into::into)
	...
	pub fn set_session_tickets_callback<F: TicketCallback>(&mut self, cb: &'c mut F) {
		unsafe{ssl_conf_session_tickets_cb(&mut self.inner, Some(F::call_write), Some(F::call_parse), cb.data_ptr())};
	}
	...
	pub fn set_sni_callback<F: FnMut(&mut HandshakeContext, &[u8]) -> Result<(),()>>(&mut self, cb: &'c mut F) {
		unsafe extern "C" fn sni_callback<F: FnMut(&mut HandshakeContext, &[u8]) -> Result<(),()>>(closure: *mut c_void, ctx: *mut ssl_context, name: *const c_uchar, name_len: size_t) -> c_int {
	...
	pub fn set_verify_callback<F>(&mut self, cb: &'c mut F)
		where F: FnMut(&mut LinkedCertificate, i32, &mut VerifyError) -> ::Result<()>
	{
		unsafe extern "C" fn verify_callback<F>(closure: *mut c_void,
		                                        crt: *mut x509_crt,
		                                        depth: c_int,
		                                        flags: *mut u32)
		                                        -> c_int
		...
		{
			let cb = &mut *(closure as *mut F);
			let crt: &mut LinkedCertificate = ::private::UnsafeFrom::from(crt).expect("valid certificate");
		...
		unsafe { ssl_conf_verify(&mut self.inner, Some(verify_callback::<F>), cb as *mut F as _) }
	}
}
