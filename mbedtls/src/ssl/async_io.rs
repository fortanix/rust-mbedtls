    ...
    for (&'a mut TaskContext<'b>, &'c mut IO)
{
    fn recv(&mut self, buf: &mut [u8]) -> Result<usize> {
        ...
        match io.poll_read(self.0, &mut buf) {
            ...
            Poll::Ready(Err(_)) => Err(Error::from(LoError::NetRecvFailed)),
            Poll::Pending => Err(Error::from(LoError::SslWantRead)),
        }
    }
    ...
    fn send(&mut self, buf: &[u8]) -> Result<usize> {
        ...
        match io.poll_write(self.0, buf) {
            Poll::Ready(Err(_)) => Err(Error::from(LoError::NetSendFailed)),
            Poll::Ready(Ok(n)) => Ok(n),
            Poll::Pending => Err(Error::from(LoError::SslWantWrite)),
        }
    }
}
