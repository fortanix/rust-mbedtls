		...
		where S: Serializer
	{
		let saved_raw_cipher = unsafe {
			...
			let algorithm_ctx = match (cipher_id, cipher_mode) {
				...
				(CIPHER_ID_DES, MODE_CFB) => AlgorithmContext::Des(Bytes(*(cipher_context.cipher_ctx as *const des_context))),
				(CIPHER_ID_3DES, MODE_CBC) |
				(CIPHER_ID_3DES, MODE_CTR) |
				(CIPHER_ID_3DES, MODE_CFB) => AlgorithmContext::Des3(Bytes(*(cipher_context.cipher_ctx as *const des3_context))),
				_ => return Err(ser::Error::custom("unsupported algorithm for serialization")),
			};
		...
		where D: Deserializer<'de>
	{
		...
		let (raw, padding) = match saved_cipher {
			SavedCipher::Encryption(..) if !Op::is_encrypt() => {
				return Err(de::Error::invalid_value(Unexpected::Other("incorrect cipher operation"), &"encryption"));
			}
			SavedCipher::Decryption(..) if Op::is_encrypt() => {
				return Err(de::Error::invalid_value(Unexpected::Other("incorrect cipher operation"), &"decryption"));
			}
		...
		let mut raw_cipher = match raw::Cipher::setup(raw.cipher_id.into(), raw.cipher_mode.into(), raw.key_bit_len) {
			Ok(raw) => raw,
			Err(_) => return Err(de::Error::invalid_value(Unexpected::Other("bad cipher parameters"), &"valid parameters")),
		};

		if raw.cipher_mode == MODE_CBC {
			try!(raw_cipher.set_padding(padding).map_err(|_| de::Error::invalid_value(Unexpected::Other("bad padding mode"), &"valid mode")));
		}
		...
		unsafe {
			...
			match (raw.cipher_id, raw.algorithm_ctx) {
				...
				_ => {
					return Err(de::Error::invalid_value(Unexpected::Other("bad algorithm"), &"valid algorithm"));
				}
			};
