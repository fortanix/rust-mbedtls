...
use crate::error::{IntoResult, Result, Error, MbedErrorCode};
...
impl Cipher {
    ...
    pub fn update(&mut self, indata: &[u8], outdata: &mut [u8]) -> Result<usize> {
        ...
        if outdata.len() < reqd_size {
            return Err(Error::from(MbedErrorCode::CipherFullBlockExpected));
        }
    ...
    pub fn encrypt_auth(
        ...
        if cipher_and_tag.len()
            ...
            return Err(Error::from(MbedErrorCode::CipherBadInputData));
        }
