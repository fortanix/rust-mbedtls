...
impl Cipher {
	...
	pub fn setup(cipher_id: CipherId, cipher_mode: CipherMode, key_bit_len: u32) -> ::Result<Cipher> {
		...
		unsafe {
			...
			try!(cipher_setup(&mut ret.inner,
							  cipher_info_from_values(cipher_id.into(), 
							  						  key_bit_len as i32, 
							  						  cipher_mode.into())
			).into_result());
		}
	...
	pub fn set_key(&mut self, op: Operation, key: &[u8]) -> ::Result<()> {
		unsafe {
			cipher_setkey(&mut self.inner, key.as_ptr(), (key.len() * 8) as _, op.into()).into_result_discard()
		}
	}

	pub fn set_padding(&mut self, padding: CipherPadding) -> ::Result<()> {
		unsafe {
			cipher_set_padding_mode(&mut self.inner, padding.into()).into_result_discard()
		}
	}
	...
	pub fn set_iv(&mut self, iv: &[u8]) -> ::Result<()> {
		unsafe {
			cipher_set_iv(&mut self.inner, iv.as_ptr(), iv.len()).into_result_discard()
		}
	}

	pub fn reset(&mut self) -> ::Result<()> {
		unsafe {
			cipher_reset(&mut self.inner).into_result_discard()
		}
	}
	...
	pub fn update(&mut self, indata: &[u8], outdata: &mut [u8]) -> ::Result<usize> {
		...
		let mut olen = 0;
		unsafe {
			try!(cipher_update(&mut self.inner, 
							   indata.as_ptr(), 
							   indata.len(),
							   outdata.as_mut_ptr(),
							   &mut olen
	...
	pub fn finish(&mut self, outdata: &mut [u8]) -> ::Result<usize> {
		...
		let mut olen = 0;
		unsafe {
			try!(cipher_finish(&mut self.inner, 
							   outdata.as_mut_ptr(),
							   &mut olen
			).into_result());
		}
	...
	pub fn check_tag(&mut self, tag: &[u8]) -> ::Result<()> {
		unsafe {
			cipher_check_tag(&mut self.inner, tag.as_ptr(), tag.len()).into_result_discard()
		}
	}
	...
	pub fn set_parity(key: &mut [u8]) -> ::Result<()> {
		unsafe {
			des_key_set_parity(key.as_mut_ptr())
		}
		Ok(())
	}
	...
	pub fn encrypt_auth(&mut self, ad: &[u8], plain: &[u8], cipher: &mut [u8], tag: &mut [u8]) -> ::Result<usize> {
		...
		unsafe { cipher_auth_encrypt(
			&mut self.inner,
			iv.as_ptr(), iv_len,
			ad.as_ptr(), ad.len(),
			plain.as_ptr(), plain.len(),
			cipher.as_mut_ptr(), &mut cipher_len,
	...
	pub fn decrypt_auth(&mut self, ad: &[u8], cipher: &[u8], plain: &mut [u8], tag: &[u8]) -> ::Result<usize> {
		...
		unsafe { cipher_auth_decrypt(
			&mut self.inner,
			iv.as_ptr(), iv_len,
			ad.as_ptr(), ad.len(),
			cipher.as_ptr(), cipher.len(),
			plain.as_mut_ptr(), &mut plain_len,
			tag.as_ptr(), tag.len()
		).into_result()? };
...
fn one_part_ecb() {
	...
	c.set_key(Operation::Encrypt, b"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f").unwrap();
	let mut out = [0u8; 48];
