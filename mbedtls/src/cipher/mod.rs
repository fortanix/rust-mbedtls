...
impl Operation for Encryption {
    fn is_encrypt() -> bool { true }
}

pub enum Decryption {}
impl Operation for Decryption {
    fn is_encrypt() -> bool { false }
}
...
impl<Op: Operation, T: Type> Cipher<Op, T, Fresh> {
	fn set_key_and_maybe_iv(&mut self, key: &[u8], iv: Option<&[u8]>) -> ::Result<()> {
		let cipher_op = if Op::is_encrypt() { raw::Operation::Encrypt } else { raw::Operation::Decrypt };
...
impl Cipher<Encryption, Authenticated, AdditionalData> {
	pub fn encrypt_auth(mut self,
	                    ad: &[u8],
	                    plain_text: &[u8],
	                    cipher_text: &mut [u8],
	                    tag: &mut [u8])
	                    -> ::Result<(usize, Cipher<Encryption, Authenticated, Finished>)> {
		Ok((self.raw_cipher.encrypt_auth(ad, plain_text, cipher_text, tag)?, self.change_state()))
	}
}

impl Cipher<Decryption, Authenticated, AdditionalData> {
	pub fn decrypt_auth(mut self,
	                    ad: &[u8],
	                    cipher_text: &[u8],
	                    plain_text: &mut [u8],
	                    tag: &[u8])
	                    -> ::Result<(usize, Cipher<Decryption, Authenticated, Finished>)> {
		Ok((self.raw_cipher.decrypt_auth(ad, cipher_text, plain_text, tag)?, self.change_state()))
	}
}
...
fn ccm() {
	// Example vector C.1
	let k = [0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f];
	...
	cipher.encrypt_auth(&ad, &p, &mut c_out, &mut t_out).unwrap();
	assert_eq!(c, c_out);
	assert_eq!(t, t_out);
	let cipher = Cipher::<_, Authenticated, _>::new(raw::CipherId::Aes, raw::CipherMode::CCM, (k.len() * 8) as _).unwrap();
	let cipher = cipher.set_key_iv(&k, &iv).unwrap();
