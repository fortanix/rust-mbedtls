...
use crate::error::{Error, IntoResult, Result, HighLevelError};
use mbedtls_sys::*;
...
impl EcGroup {
    ...
    pub fn from_parameters(
        ...
        if &a < &zero
            ...
            || (&a == &zero && &b == &zero)
        {
            return Err(Error::from(HighLevelError::EcpBadInputData));
        }
    ...
    pub fn cofactor(&self) -> Result<u32> {
        match self.group_id()? {
            ...
            EcGroupId::None => Err(Error::from(HighLevelError::EcpFeatureUnavailable)),
            _ => Ok(1),
        }
    }
...
impl EcPoint {
    ...
    pub fn from_binary(group: &EcGroup, bin: &[u8]) -> Result<EcPoint> {
        let prefix = *bin.get(0).ok_or(Error::from(HighLevelError::EcpBadInputData))?;
    ...
    pub fn is_zero(&self) -> Result<bool> {
        ...
        match unsafe { ecp_is_zero(&self.inner as *const ecp_point as *mut ecp_point) } {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(Error::from(HighLevelError::EcpInvalidKey)),
        }
    }
    ...
    pub fn muladd(
        ...
        if group.contains_point(&pt1)? == false {
            return Err(Error::from(HighLevelError::EcpInvalidKey));
        }
