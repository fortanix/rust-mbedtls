...
pub fn criterion_benchmark(criterion: &mut Criterion) {
    ...
    let expected_tag: [u8; 4] = [0x4d, 0xac, 0x25, 0x5d];
    let cipher_and_tag = aes_ccm_encrypt(&key, &iv, &ad, &plain_text, &expected_cipher, &expected_tag);
    ...
    criterion.bench_function("AES CCM decrypt", |b| {
        b.iter(|| aes_ccm_decrypt(&key, &iv, &ad, black_box(&cipher_and_tag), &plain_text))
    });
}

fn aes_ccm_decrypt(key: &[u8; 16], iv: &[u8; 7], ad: &[u8; 8], cipher_and_tag: &[u8; 8], expected_plain: &[u8; 4]) -> [u8; 4] {
    let mut plain_out = [0u8; 4];
    ...
    cipher.decrypt_auth(ad, cipher_and_tag, &mut plain_out, 4).unwrap();
